...
void AtmosphereSample::Initialize(const SampleInitInfo& InitInfo)
{
  ...
  const Char *strTileTexPaths[EarthHemsiphere::NUM_TILE_TEXTURES],
    *strNormalMapPaths[EarthHemsiphere::NUM_TILE_TEXTURES];
  ...
  m_pLightSctrPP.reset(new EpipolarLightScattering(m_pDevice, m_pImmediateContext, SCDesc.ColorBufferFormat,
                                                   SCDesc.DepthBufferFormat, TEX_FORMAT_R11G11B10_FLOAT));
    ...
    if (m_bEnableLightScattering)
    {
      if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None))
      {
        if (ImGui::BeginTabItem("Basic"))
        {
          ...
          static_assert(LIGHT_SCTR_TECHNIQUE_EPIPOLAR_SAMPLING == 0 && LIGHT_SCTR_TECHNIQUE_BRUTE_FORCE == 1,
                        "Unexpected value");
          ImGui::Combo("Light scattering tech", &m_PPAttribs.iLightSctrTechnique,
                       "Epipolar\0"
                       "Brute force\0\0");
          ...
          if (m_PPAttribs.iLightSctrTechnique == LIGHT_SCTR_TECHNIQUE_EPIPOLAR_SAMPLING)
          {
            ...
            ImGui::HelpMarker(
              "Whether to correct inscattering at depth discontinuities. Improves quality for additional cost.");

            if (m_PPAttribs.bCorrectScatteringAtDepthBreaks)
            {
              ImGui::Checkbox("Show Depth Breaks", &m_PPAttribs.bShowDepthBreaks);
              ImGui::HelpMarker(
                "Whether to display pixels which are classified as depth discontinuities and which will be corrected.");
            }
          }
        ...
        if (ImGui::BeginTabItem("Advanced"))
        {
          ...
          if (ImGui::InputFloat("Aerosol Density", &m_PPAttribs.fAerosolDensityScale, 0.1f, 0.25f, "%.3f",
                                ImGuiInputTextFlags_EnterReturnsTrue))
            m_PPAttribs.fAerosolDensityScale = clamp(m_PPAttribs.fAerosolDensityScale, 0.1f, 5.0f);

          if (ImGui::InputFloat("Aerosol Absorption", &m_PPAttribs.fAerosolAbsorbtionScale, 0.1f, 0.25f, "%.3f",
                                ImGuiInputTextFlags_EnterReturnsTrue))
        ...
        {
          {
            ...
            ImGui::Combo("Tone Mapping Mode", &m_PPAttribs.ToneMapping.iToneMappingMode, ToneMappingMode.data(),
...
void AtmosphereSample::Render()
{
  ...
  m_pImmediateContext->ClearRenderTarget(pRTV, m_bEnableLightScattering ? Zero : ClearColor,
                                         RESOURCE_STATE_TRANSITION_MODE_TRANSITION);
  ...
  m_TerrainRenderParams.DstRTVFormat =
  ...
  if (m_bEnableLightScattering)
  {
    ...
    m_PPAttribs.f2ShadowMapTexelSize = float2(1.f / static_cast<float>(m_ShadowSettings.Resolution),
                                              1.f / static_cast<float>(m_ShadowSettings.Resolution));
    ...
    m_PPAttribs.uiInitialSampleStepInSlice =
      std::min(m_PPAttribs.uiInitialSampleStepInSlice, m_PPAttribs.uiMaxSamplesInSlice);
    m_PPAttribs.uiEpipoleSamplingDensityFactor =
      std::min(m_PPAttribs.uiEpipoleSamplingDensityFactor, m_PPAttribs.uiInitialSampleStepInSlice);
                                        ...
                                        float&          fFarPlaneZ)
{
  ...
  if (fCameraElev > fMaxRadius)
  {
    ...
    fNearPlaneZ =
      (fCameraElev - fMaxRadius) / sqrt(1 + 1.f / (ProjMatr._11 * ProjMatr._11) + 1.f / (ProjMatr._22 * ProjMatr._22));
  }
  ...
  for (int i = 0; i < iNumTestDirections; ++i)
  {
    for (int j = 0; j < iNumTestDirections; ++j)
    {
      ...
      float fNearIsecWithBottomSphere =
        IsecsWithBottomBoundSphere.x > 0 ? IsecsWithBottomBoundSphere.x : IsecsWithBottomBoundSphere.y;
...
void AtmosphereSample::Update(double CurrTime, double ElapsedTime)
{
  ...
  m_CameraRotation = Quaternion::RotationFromAxisAngle(float3{1, 0, 0}, -m_fCameraPitch) *
  ...
  if ((m_LastMouseState.ButtonFlags & MouseState::BUTTON_FLAG_RIGHT) != 0)
  {
    ...
    m_f3LightDir = float4(m_f3LightDir, 0) * float4x4::RotationArbitrary(WorldUp, fYawDelta) *
